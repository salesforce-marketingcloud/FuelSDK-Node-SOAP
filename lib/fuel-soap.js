...
FuelSoap.prototype.schedule = function(type,schedule,interactions, action, options, callback) {
	...
	if(typeof body.ScheduleRequestMsg.Schedule.Recurrence === "object"){
	        if(body.ScheduleRequestMsg.Schedule.Recurrence.type){
	            body.ScheduleRequestMsg.Schedule.Recurrence.$ = {"xsi:type": body.ScheduleRequestMsg.Schedule.Recurrence.type};
	        }else{
	            throw new TypeError("Recurrence type is not defined");
	        }
        }else {
	    throw new TypeError("Recurrence must be of Object Type");
        }
