	...
	this.AuthClient.getAccessToken(clone(options.auth), function(err, body) {
		...
		request(requestOptions, function(err, res, body) {
			...
			this._parseResponse(options.key, body, function(err, data) {
				...
				if(err) {
					helpers.deliverResponse('error', err, callback, 'soapRequest > request > _parseResponse');
				} else {
...
FuelSoap.prototype.create = function(type, props, options, callback) {
	...
	optionsAndCallback = determineCallbackAndOptions(arguments, options, callback);
...
function determineCallbackAndOptions(args, options, callback) {
	if(args.length < 4) {
